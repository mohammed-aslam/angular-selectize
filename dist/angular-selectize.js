        ...
        link: function (scope, element, attrs, modelCtrl) {
            ...
            var setSelectizeOptions = function (curr, prev) {
                for (var i in prev) {
                    ...
                    for (var j in curr) {
                        var currOpt = curr[j];
                        if (currOpt[settings.valueField] === prevOpt[settings.valueField]) {
                            if (!angular.equals(currOpt, prevOpt)) {
                                selectize.updateOption(currOpt[settings.valueField], currOpt);
                            }
                            prevOpt = null;
                            break;
                        }
                    ...
                    if (prevOpt) {
                        selectize.removeOption(prevOpt[settings.valueField], true);
                    }
                }
            ...
            settings.onInitialize = function () {
                ...
                scope.$watch('options', setSelectizeOptions, true);
